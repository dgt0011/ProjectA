...
[Authorize]
public class EditModel(IBookmarkTypesApiClient bookmarkTypesApiClient) : PageModel
{
    ...
    {
        var result = await bookmarkTypesApiClient.GetByIdAsync(Id, cancellationToken);
        if (!result.IsSuccess || result.Value is null)
        {
            TempData["ErrorMessage"] = result.ToDisplayMessage("Bookmark type not found.");
            return RedirectToPage("Index");
        }

        Form = new BookmarkTypeInput { Title = result.Value.Title, Color = result.Value.Color };
    ...
    public async Task<IActionResult> OnPostAsync(CancellationToken cancellationToken)
    {
        ...
        var result = await bookmarkTypesApiClient.UpdateAsync(Id, Form, cancellationToken);
        if (!result.IsSuccess)
        {
            ModelState.AddApiErrors(result.Errors);
            if (result.Errors is null)
            {
                ModelState.AddModelError(string.Empty, result.ToDisplayMessage("Could not update the bookmark type."));
            }
        ...
        TempData["SuccessMessage"] = "Bookmark type updated.";
        return RedirectToPage("Index");
    }
